...
#if defined(NDEBUG)
# error "PetaCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xddcc74f31e7bc1210ea5501f151bd5c183c6fb16a22ae05fa982406e3d855325");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PetaCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PetaCoin Signed Message:\n";
...
{
    // PetaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 300000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 300000); // PetaCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 7 * 60; // PetaCoin: 7 minutes
static const int64 nTargetSpacing = 7 * 60; // PetaCoin: 7 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PetaCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PetaCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5ede0e3f7043b5b65132610769a4456147b2564ab01152857309db8f0edcf961");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Onet News: 07.08.2022 A Russian politician threatens the German Bild Journalist in Ukraine with death";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e36db27b46cbbede11ffdce4d00563be98f036c5fa6b0faece4f697dd965d567b2005ee00a174ed8ad623385a5c67e766bb45c6a5d4fd0b05f7d17752eec756e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1659889930;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 423593;

        if (fTestNet)
        {
            block.nTime    = 1659889900;
            block.nNonce   = 1012240;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x473acac7292773fe806851be05f332a28e8dcabb6969c8af861a149693ca0761"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb2, 0xd1 }; // PetaCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PetaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PetaCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PetaCoinMiner(CWallet *pwallet)
{
    printf("PetaCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("PetaCoin-miner");
    ...
    try { loop {
        ...
        printf("Running PetaCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PetaCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PetaCoinMiner, pwallet));
}
